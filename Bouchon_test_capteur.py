...
def generate_fake_data():
    pluviometer_value = round(random.uniform(0, 20), 2)  # Valeur aléatoire pour le pluviomètre entre 0 et 20 mm
    limnimeter_value = round(random.uniform(0, 10), 2)    # Valeur aléatoire pour le limnimètre entre 0 et 10 mètres
...
def insert_fake_data():
    ...
    VALUES (%s, %s, %s, %s)
    ''', (1, pluviometer_value, 'mm', current_time))  #Capteur numéro 1, valeur généré aléatoirement, unité et la date.
    ...
    conn.commit()
    print(f"Pluviomètre: {pluviometer_value} mm, Limnimètre: {limnimeter_value} m a {current_time}")
